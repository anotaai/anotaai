...
public class SessaoUsuarioService {
	...
	public ResponseEntity<?> isActive(String sessionID) {
		...
		if (!responseEntity.getIsValid()) {
			responseEntity.addMessage(IMessage.SECURITY_SESSION_TIMEOUT, TipoMensagem.ERROR, Constant.App.LONG_TIME_VIEW);
		}
	...
	public SessaoUsuario getSessionActive(String sessionID) throws AppException {
		...
		try {
			...
			responseEntity.addMessage(IMessage.SECURITY_SESSION_TIMEOUT, TipoMensagem.ERROR, Constant.App.DEFAULT_TIME_VIEW);
